...
class RideHitch:
    ...
    def encode_state_4(self):
        self.state_num = 1 + self.pool_size
        state = np.zeros(self.state_num)
        for i, supply in enumerate(self.supply_pool):
            if check_match(supply, self.latest_request, self.T_threshold, self.D_threshold):
                state[i] = supply[cap_idx]
            else:
                state[i] = 0.
        state[-1] = self.latest_request[cap_idx]
        return state
