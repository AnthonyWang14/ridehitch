...
class DeepQNetwork:
    ...
    def choose_action(self, observation):
        ...
        # combine the rules to choose action
        if np.random.uniform() < self.epsilon:
            # forward feed the observation and get q value for every actions
            actions_value = self.sess.run(self.q_eval, feed_dict={self.s: observation})
            rule_actions_value = np.zeros(self.n_actions)
            ...
            for j in range(6):
                demand[1+j] = observation[0][6*self.n_actions+j]
            # print(actions_value[0])
            for i in range(self.n_actions):
                supply_chosen = [0,0,0,0,0,0,0]
                for j in range(6):
                    supply_chosen[1+j] = observation[0][6*i+j]
                if check_match(supply_chosen, demand, self.T_threshold, self.D_threshold):
